...
def _do_search_and_match(
    ...
    if videoId:
        ...
        try:
            ...
            #     videoId_results, song_name, song_artists, song_title, song_duration
            # )
    ...
    ordered_song_info_results = _order_yt_results(
        song_info_results, song_name, song_artists, song_title, song_duration
    )
    ...
    if len(ordered_song_info_results) == 0:
        if bracked_re.search(song_name) or any(
            ...
            # ordered_song_info_results = _order_yt_results(
            #     converted_results, song_name, song_artists, song_title, song_duration
            # )
...
def _order_yt_results(
    ...
    for result in results:
        ...
        if (
            result.get("category") == "Videos"
            and result.get("resultType") == "video"
        ):
            name_match = round(
                _match_percentage(
                    str(unidecode(song_title)),
            ...
            if name_match:
                videoIds_with_match_value.append(
                    {"average_match": name_match, "result": result}
                )

        else:
            lower_song_name = song_name.translate(
            ...
            sentence_words = lower_song_name.split(" ")
            ...
            if (
                song_duration
                and song_duration > 0
                and "duration_seconds" in result
            ):
                delta = (
                    result["duration_seconds"] - song_duration
                )  # ! check this
